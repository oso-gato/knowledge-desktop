...
**Vault (E4) — BUILT (WP-17 MVP core):** central git repo (`box.vault_repo`), per-user `~/Vault` clones, 5-min sync cycle (`shared/kd-vault-sync`, lineage-agnostic): stop-flag → guarded commit (bulk-delete bound `max(25,10%)`) → guarded fetch/merge (same bound inbound; **conflict ⇒ `merge --abort`, byte-identical restore, stop-flag, bus-observed desktop notification**) → plain push. Every step is FAIL-SAFE (a transient/unreachable/rejected op leaves the tree intact + returns 0 → next cycle retries); only a genuine danger (mass-delete either direction, or a conflict) sets the durable human-cleared stop-flag. **Driver is per-lineage:** L1 (bash-PID-1, no systemd `--user`) runs `kd-vault-driver`, an entrypoint-supervised loop that sweeps every provisioned user's `~/Vault` each interval and fans out `kd-vault-sync` per user (one user's failure never aborts the sweep); L2 uses a per-user systemd timer (rides WP-08). `kd-vault-init` (run per user at provision by kd-provision, gated on `box.vault_repo`; absent ⇒ vault disabled + logged, E2 degrade) clones, seeds the `.gitignore`, and asserts remote protection. Server-side no-rewrite: production GitHub ruleset (block force-push + restrict deletions, no bypass); gate mirrors it with `receive.denyNonFastForwards`+`denyDeletes` on a local bare remote. **Ruleset assertion is fail-SAFE, not fail-fast** (challenge VST-#8), implemented as a self-healing readiness marker `~/.kd/vault-ready`: sync runs ONLY when the remote is affirmatively confirmed protected (local-path bare → read `receive.deny*`; GitHub URL → `gh api …/rulesets`); unreachable/unverified/affirmatively-unprotected all WITHHOLD vault-ready (sync paused, distinct loud alert, box serves) — and because vault-ready is re-evaluated every boot, a transient outage self-heals with no human action (the human stop-flag is reserved for danger). A **local-path** `vault_repo` (the gate mirror / a local deployment) is trusted via a *scoped* git `safe.directory` for exactly that path (the roster is the trust root; cross-uid clones of a shared bare would otherwise be refused); production https/ssh URLs never take that branch. `.gitignore` broadened to all per-device volatile `.obsidian` state; two-user config-churn soak test added (challenge VST-#15). **In-box PROVEN**: 41-case engine-free suite (`shared/tests/kd-vault.test.sh` — clean sync, stop-flag + vault-ready gates, local+inbound mass-delete guards, conflict abort+byte-identical-restore, ff + divergent-clean merge, unreachable fail-safe, ruleset confirmed/absent/unverified, `.gitignore` effect, driver selection+fail-safe) + a b3-container integration (kd-provision → per-user clone/owned/`.gitignore`/vault-ready + a real per-user push round-trip). DEFERRED honestly to host-gate (needs a gate-harness fenced bare-remote fixture + roster `vault_repo`, a fedora-bootstrap-side extension like the WP-02 PR2-7 arc): the live `+R` round-trip probe + the config-churn soak on a real fenced remote. E6 additions: the delete bound is client-path-only (server layer covers history rewrite only); in-app cloud sync (Obsidian Sync/Firefox Sync) runs as the vault owner and is an owner-account-class residual; the "mechanically unable" cloud-sync template is today satisfied vacuously (no general sync service ships) — stated plainly, plus a gate lint that no system unit reaches /home outside the template.
